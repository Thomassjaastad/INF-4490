...
def TournamentSelection(lamb, k):  #k < lamb. lamb is a subset of population
	...
	while current_member <= lamb:
		...
		#print (individual)		
		mating_pool.append(individual[np.argmax(EvalFitness)])
		current_member += 1
	#print (individual)
	return mating_pool
...
candidate = TournamentSelection(4, 2)
Parents = np.zeros((len(candidate), n))

for i in range(len(candidate)):
	Parents[i] = Mutation(candidate[i])[0]


for parent in Parents:	
	offspring1 = (PMX(Parents[0], Parents[1], Mutation(Parents[0])[1], Mutation(Parents[1])[2]))


print(offspring1)
print(Parents)
New_generation = np.append([offspring1], Parents, axis = 0)
